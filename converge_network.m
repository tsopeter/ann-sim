% network
% similar to para network but tests convergence speed
clc;
clear;
...
N_RUNS    = 12;          % number of runs per nonlinearity, used for averaging (only active when run_stats is true)
...
ppContainer(length(PPS),length(NumEpochs))=Container();
for uu=1:length(NumEpochs)
    numEpochs=NumEpochs(uu);
    for j=CONT
        pp2=PPS{j};
        ...
        %% statistics
            ...
            ppContainer(j,uu).acc = acc;
            ppContainer(j,uu).avg = avg_accuracy;
            ppContainer(j,uu).sdv = sdv_accuracy;
            ppContainer(j,uu).var = var_accuracy;
        else
            net = trainNetwork(trainingData, lgraph, options);
            YPred = classify(net,testingData);
            YValidation = testingData.Labels;
...
if run_stats
    ...
    hold on;
    lgnds=[];
    for i=1:size(ppContainer, 1)
        avg_accum = zeros(size(ppContainer, 2), 1);
        for j=1:size(ppContainer, 2)    % accumulate average into N-by-1 vector
            avg_accum(j) = ppContainer(i,j).avg;
        end
        plot(avg_accum);
        lgnds=[lgnds;ppContainer(i,1).Id+" Average"];
    end
    title("Converged Accuracy vs Number of Epochs");
